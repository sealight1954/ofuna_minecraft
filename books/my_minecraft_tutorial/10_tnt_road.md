...
`ブロックを並べる`コマンドができました！次の節で、`ブロックを並べる`コマンドを練習してみましょう。
...
4. 位置の`ひがし`(`ひだり`)を`1`から`5`にする
![](/images/10_tnt_road/2024-01-07-07-20-47.png)
*道路の長さが5(1から5)になる。*
これが、道路の長さになります。

1. 位置の`たかさ`(`まんなか`)を、`-1`から`-1`にする
![](/images/10_tnt_road/2024-01-07-07-21-34.png)
*道路の高さが-1(-1から-1)になる*
これが、道路の高さになります。

1. 位置の`みなみ`(`みぎ`)を、`1`から`4`にする
![](/images/10_tnt_road/2024-01-08-06-17-06.png)
*道路のはばが4(1から4)になる*
これが、道路のよこはばになります。

石の道路を作るプログラム`tntroad`ができました！`t`キーを押してチャット画面を開き、コマンドを実行してみましょう。

![](/images/10_tnt_road/2024-01-08-06-42-06.png)
*石の道路ができる*

## STEP 2/3 `かんたんなTNT道路` TNTを置く
石の道路をおいたところに、TNTの道路をあとからおきます。コピー＆ペーストをしてみましょう。
1. 前の節で作った`石ブロックを並べる`コマンドをクリックする
![](/images/10_tnt_road/2024-01-08-06-35-10.png)
*クリックされると きいろ になる*

2. `Ctrl-C`(コピー)を押す
3. `Ctrl-V`(ペースト)を押す
![](/images/10_tnt_road/2024-01-08-06-36-46.png)
*同じコードブロックができる*

4. できたコードブロックをドラッグ&ドロップして`tntroad`プログラムの中に配置する
![](/images/10_tnt_road/2024-01-08-06-37-32.png)

5. ブロックの種類をTNTに変える
![](/images/10_tnt_road/2024-01-08-06-38-28.png)
*tntと入力するとしぼりこめる*
![](/images/10_tnt_road/2024-01-08-06-39-14.png)
*TNTを置く*

6. 位置の`みなみ`(`みぎ`)を、`2`から`3`にする
![](/images/10_tnt_road/2024-01-08-06-42-42.png)
*TNT道路のはばが2(2から3)になる*
これが、TNTのよこはばになります。


石とTNTでできた**TNT道路**を作るプログラム`tntroad`ができました！`t`キーを押してチャット画面を開き、コマンドを実行してみましょう。

![](/images/10_tnt_road/2024-01-08-06-46-40.png)

## STEP 3/3 `かんたんなTNT道路` 問題: 道路を長くしよう


# `はばのひろいTNT道路`

## STEP 1/2 `はばのひろいTNT道路` 
